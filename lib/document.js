...
function Document(model, options) {
    ...
    */
    ...
    // links to hasOne/hasMany documents -- we use
    // We use it to know if some links have been removed/added before saving.
...
Document.prototype.validate = function(options, prefix) {
    ...
    if (typeof this._getModel()._validator === 'function') {
        this._getModel()._validator.apply(this);
    }

    // Validate this schema
    this._validate(this, schema, prefix, docOptions, this)
    ...
    for(key in iterator) {
        ...
        for(var i=0; i<types.length; i++) {
            ...
            if (currentSchema === className) {
                ...
                fieldChecked = true;
                if (typeof currentSchema.validator === 'function') {
                    currentSchema.validator(doc[key]);
                }
                else {
                    if (doc[key] === undefined) {
                        if (localOptions.enforce_missing === true) {
        ...
        if (fieldChecked === false) { // The field is not a primitive, let's keep looking
            if ((currentSchema === Date) || (util.isPlainObject(currentSchema) && currentSchema._type === Date)) {
                if ((util.isPlainObject(currentSchema)) && (typeof currentSchema.validator === 'function')) {
                    currentSchema.validator.apply(doc[key])
                }
                else {
                    if (doc[key] === undefined) {
                        if (localOptions.enforce_missing === true) {
                        ...
                        if (date.getTime() === date.getTime()) {
                            ...
                            doc[key] = r.ISO8601(doc[key]); // Use r.ISO8601 and not `new Date()` to keep timezone
                        }
                        else {
                            if (localOptions.enforce_type === "strict") {
                                util.strictType(prefix, key, "date or a valid string");
                            }
                            else if (localOptions.enforce_type !== "none") {
                                util.looseType(prefix, key, "date or a valid string");
                            }
                        }
                    }
            ...
            else if (Array.isArray(currentSchema) || (util.isPlainObject(currentSchema) && currentSchema._type === Array)) {
                if (doc[key] === undefined) {
                    if (localOptions.enforce_missing === true) {
                        util.undefinedField(prefix, key);
                    }
                    ...
                    if (Array.isArray(currentSchema)) {
                        ...
                        Document.prototype._validate(doc[key], [currentSchema.schema], prefix+'['+key+']', localOptions)
                    }
                }
                else {
                    if (localOptions.enforce_type === "strict") {
                ...
                if (typeof currentSchema.validator === 'function') {
                    ...
                    if (doc[key] === undefined) {
                        ...
                        if (currentSchema.schema !== undefined) {
                            Document.prototype._validate(doc[key], currentSchema.schema, prefix+'['+key+']', localOptions)
                        }
                        else {
                            Document.prototype._validate(doc[key], currentSchema, prefix+'['+key+']', localOptions)
...
Document.prototype._save = function(modelToSave, saveAll, savedModel) {
    ...
    savedModel[constructor.getTableName()] = 1;
    ...
    var p = new Promise(function(resolve, reject) {
        ...
        var saveMany = function() {
            ...
            for(var key in model._joins) {
                if (((key in modelToSave) || (saveAll === true)) &&
                        (model._joins[key].type === 'hasOne') && ((saveAll === false) || (savedModel[model._joins[key].model.getTableName()] !== 1))) {

                    savedModel[model._joins[key].model.getTableName()] = 1;
            ...
            for(var key in model._joins) {
                if (((key in modelToSave) || (saveAll === true)) &&
                        (model._joins[key].type === 'hasMany') && ((saveAll === false) || (savedModel[model._joins[key].model.getTableName()] !== 1))) {
            ...
            for(var key in model._joins) {
                // Compare to null
                if (((key in modelToSave) || (saveAll === true)) &&
                        (model._joins[key].type === 'hasAndBelongsToMany') && ((saveAll === false) || (savedModel[model._joins[key].model.getTableName()] !== 1))) {
        ...
        var saveSelf = function() {
            ...
            for(var key in model._joins) {
                if (model._joins[key].type === 'belongsTo') {
                    if (self[key] != null) {
        ...
        for(var key in model._joins) {
            if (((key in modelToSave) || (saveAll === true)) &&
                    (model._joins[key].type === 'belongsTo') && (self[key] != null) && ((saveAll === false) || (savedModel[model._joins[key].model.getTableName()] !== 1))) {
...
Document.prototype._delete = function(modelToDelete, deleteAll, deletedModel, deleteSelf) {
    var self = this;
    ...
    deletedModel[constructor.getTableName()] = 1

    for(var key in joins) {
        if ((joins[key].type === 'hasOne') && (util.isPlainObject(self[key]))) {
            if (((key in modelToDelete) || (deleteAll === true)) &&
                ((deleteAll === false) || (deletedModel[joins[key].model.getTableName()] !== 1))) {

                promises.push(self[key]._delete(modelToDelete[key], deleteAll, deletedModel))
            }
            else {
            ...
            if ((key in modelToDelete) || (deleteAll === true)) {
                for(var i=0; i<self[key].length; i++) {
                    if ((self[key][i] instanceof Document) && (self[key][i].isSaved() === true)) {
                        ...
                        promises.push(self[key][i]._delete(modelToDelete[key], deleteAll, deletedModel))
                    }
                }
            }
            else {
            ...
            if (((key in modelToDelete) || (deleteAll === true)) &&
                ...
                for(var i=0; i<self[key].length; i++) {
                    if ((self[key][i] instanceof Document) && (self[key][i].isSaved() === true)) {
                        ...
                        promises.push(self[key][i]._delete(modelToDelete[key], deleteAll, deletedModel, false))
                    ...
                    (function(_key) {
                        promises.push(new Promise(function(resolve, reject) {
                            query.run().then(function() {
                                for(var i=0; i<self[_key].length; i++) {
                                    self[_key][i].emit('delete', self[_key][i]);
                                    self[_key][i]._setUnSaved();
                                }
                                resolve();
                            }).error(reject);
                        }))
