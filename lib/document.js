...
Document.prototype._save = function(modelToSave, saveAll, savedModel) {
    //TODO How should we handle circular references outsides of joined fields? Now we throw with a maximum call stack size exceed
    var self = this;
    ...
    savedModel[constructor.getName()] = 1;
            ...
            var cbBelongsTo = function() {
                for(var key in model._joins) {
                    if (((key in modelToSave) || (saveAll === true)) &&
                        ...
                        if (self[key].__proto__._parents._belongsTo[constructor.getName()] == null) {
                            self[key].__proto__._parents._belongsTo[constructor.getName()] = [];
                        }
                        self[key].__proto__._parents._belongsTo[constructor.getName()].push({
                            doc: self,
...
Document.prototype.purge = function() {
    ...
    for(var field in self._getModel()._reverseJoins) {
        ...
        if (join.type === 'belongsTo') {
            ...
            promises.push(r.table(joinedModel.getName()).getAll(self[join.rightKey], {index: join.leftKey}).replace(function(doc) {
