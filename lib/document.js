...
Document.prototype._save = function(modelToSave, saveAll, savedModel) {
    ...
    //TODO Test modelToSave
    //TODO Properly call saveAll with the right arguments
    ...
    var p = new Promise(function(resolve, reject) {
        ...
        var saveLinks = function() {
            ...
            for(var key in model._joins) {
                //TODO Replace model._joins[key].model.getName() in modelToSave with key in modelToSave
        ...
        for(var key in model._joins) {
            if (((model._joins[key].model.getName() in modelToSave) || (saveAll === true)) &&
                    (model._joins[key].type === 'belongsTo') && (self[key] != null) && ((saveAll === false) || (savedModel[model._joins[key].model.getName()] !== true))) {

                savedModel[model._joins[key].model.getName()] = true;
                if (saveAll === true) {
                    promises.push(self[key].saveAll({}, true, savedModel))
                }
                else {
                    promises.push(self[key].saveAll(modelToSave[model._joins[key].model.getName()], false, savedModel))
                }
            }
        }
        ...
        if (promises.length > 0) {
            if (model._tableReady === true) {
                //TODO Refactor Promise.all with respect to "else" block
