...
Document.prototype.__generateDefault = function(doc, schema, options, originalDoc) {
    ...
    if (util.isPlainObject(doc)) {
        for(var key in schema) {
            ...
            if ((util.isPlainObject(schema[key])) && (schema[key]._type === Object)) {
        ...
        for(var i=0; i<types.length; i++) {
            ...
            if (schema[key] === className) {
                ...
                if (doc[key] === undefined) {
                    ...
                    else if (options.enforce_type === "strict") {
            ...
            else if (util.isPlainObject(schema[key]) && (schema[key]._type === className)) {
                fieldChecked = true;
        ...
        if (fieldChecked === false) { // The field is not a primitive, let's keep looking
            if (schema[key] === Date) {
                if (doc[key] === undefined) {
                    ...
                    //TODO Check if all the fields are present
                }
                else {
                    if (options.enforce_type === "strict") {
                        util.strictType(prefix, key, "date");
