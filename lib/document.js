        ...
        if (error) {
            ...
            if (count.saved === count.toSave) {
                ...
                if ((originalDoc != null) && (joinClause != null) && (originalDoc !== doc)) {
                    originalDoc.save(callback, true);
                }
                else {
...
Document.prototype.save = function(callback, skipSub) {
    ...
    var skipSubWasUndefined = (skipSub == null)
    if (skipSub == null) {
        skipSub = false;
    }
    ...
    if (count.toSave === 0) {
        ...
        /*
        if ((skipSubWasUndefined === true) || (skipSub === true)) {
        ...
        */
        ...
        else if (callback.wrapped === true) {
            cb = callback
        }
        else {
            cb = Document.createCallback(self, callback, count)
        }
        this.execute.call(self, query, cb);
    }
