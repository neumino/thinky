...
Document.prototype._validate = function(options, modelToValidate, validateAll, validatedModel, prefix) {
    options = options || {};
    var docOptions = util.deepCopy(this._getOptions()); // Document's options
    ...
    this.__validate(this, schema, prefix, docOptions, this);
    ...
    var joins = this._getModel()._joins;
        ...
        if (((joins[key].type === 'hasOne') || (joins[key].type === 'belongsTo'))) {
            if (((key in modelToValidate) || (validateAll === true)) &&
                ...
                if (util.isPlainObject(this[key])) {
                    if (this[key] instanceof Document === false) {
                        this[key] = new this._getModel()._joins[key].model(this[key]);
                    }
                    // We do not propagate the options of this document, but only those given to validate
                    this[key]._validate(options, modelToValidate[key], validateAll, validatedModel, prefix+'['+key+']');
                }
            ...
            if (((key in modelToValidate) || (validateAll === true)) &&
                ...
                if (Array.isArray(this[key])) {
                    for(var i=0; i<this[key].length; i++) {
                        if (this[key][i] instanceof Document === false) {
                            this[key][i] = new this._getModel()._joins[key].model(this[key][i]);
                        }
                        this[key][i]._validate(options, modelToValidate[key], validateAll, validatedModel, prefix+'['+key+']['+i+']');
                    }
                }
                else if (this[key] != null) {
            ...
            if ((currentSchema === Date) || (util.isPlainObject(currentSchema) && currentSchema._type === Date)) {
                ...
                if (doc[key] === undefined) {
                    ...
                    if (Array.isArray(currentSchema)) {
                        Document.prototype.__validate(doc[key], currentSchema, prefix+'['+key+']', localOptions, originalDoc)
                    }
                    else {
                        Document.prototype.__validate(doc[key], [currentSchema.schema], prefix+'['+key+']', localOptions, originalDoc)
                    }
                }
                else {
                    if (localOptions.enforce_type === "strict") {
