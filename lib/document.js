 ...
 */
//TODO Implement validateAll, validate, _validate like save, saveAll, _save
Document.prototype.validate = function(options, prefix) {
    ...
    if (typeof this._getModel()._validator === 'function') {
        if (this._getModel()._validator.call(this, this) === false) {
...
Document.prototype._validate = function(doc, schema, prefix, options, originalDoc) {
        ...
        for(var i=0; i<types.length; i++) {
            ...
            if (currentSchema === className) {
                ...
                if (typeof currentSchema.validator === 'function') {
                    if (currentSchema.validator(doc[key]) === false) {
            ...
            if ((currentSchema === Date) || (util.isPlainObject(currentSchema) && currentSchema._type === Date)) {
                ...
                if (doc[key] === undefined) {
                    ...
                    if (Array.isArray(currentSchema)) {
                        Document.prototype._validate(doc[key], currentSchema, prefix+'['+key+']', localOptions, originalDoc)
                    }
                    else {
                        Document.prototype._validate(doc[key], [currentSchema.schema], prefix+'['+key+']', localOptions, originalDoc)
                    }
                }
                else {
                    if (localOptions.enforce_type === "strict") {
...
Document.prototype._save = function(modelToSave, saveAll, savedModel) {
    ...
    var p = new Promise(function(resolve, reject) {
            ...
            if (self.__proto__._saved === false) {
                var savedDocCb = function(result) {
                    try{ // Validate the doc, replace it, and tag it as saved
                        Document.prototype._validate(result.new_val, model._schema, '', model._options, self)
                ...
                var saveSelfHelper = function() {
                    try{
                        ...
                        Document.prototype._validate(copy, model._schema, '', model._options, self)
        ...
        for(var key in model._joins) {
            if (((key in modelToSave) || (saveAll === true)) &&
                    (model._joins[key].type === 'belongsTo') && ((saveAll === false) || (savedModel[model._joins[key].model.getTableName()] !== 1))) {
                        ...
                        promises.push(new Promise(function(resolve, reject) {
                            query.run().then(function() {
                                for(var i=0; i<docsToDelete.length; i++) {
                                    docsToDelete[i].emit('delete', docsToDelete[i]);
