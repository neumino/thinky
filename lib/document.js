      ...
      if (copy[model._pk] === undefined) {
        ...
        return querySaveSelf = r.table(constructor.getTableName())
            .get(copy[model._pk]).replace(function (row) {
              ...
              return r.branch(row(model._version).eq(existingVersion), copy, r.error("Versioning failed - there is a different version for " + constructor.getTableName() + " in the database from: " + existingVersion));
