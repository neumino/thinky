...
Document.prototype._saveVirtual = function() {
    ...
    // could exist there.
    for(var key in this) {
        if ((this.hasOwnProperty(key)) && (model._joins[key] === undefined)) {
            copy[key] = this[key];
        }
    }
...
Document.prototype._makeSavableCopy = function() {
    ...
    return this.__makeSavableCopy(this, schema, model, r)
}
// __makeSavableCopy
Document.prototype.__makeSavableCopy = function(doc, schema, model, r) {
    ...
    if (util.isPlainObject(doc) && (doc instanceof Buffer === false)) {
        ...
        for(var i=0; i<keys.length; i++) {
            ...
            if ((util.isPlainObject(model) === false) || (model._joins[key] === undefined)) { // We do not copy joined documents
                if (util.isPlainObject(schema) === false) {
                ...
                if (copyFlag === true) {
                    result[key] = Document.prototype.__makeSavableCopy(doc[key], nextSchema, undefined, r);
                }
            }
        }
        if (util.isPlainObject(model) === true) {
            // Copy the fields that are used as foreign keys
            for (var localKey in model._localKeys) {
                if (doc[localKey] !== undefined) {
                    result[localKey] = Document.prototype.__makeSavableCopy(doc[localKey], nextSchema, undefined, r);
                }
            }
        }
        return result;
        ...
        if (copyFlag === true) {
            for(var i=0; i<doc.length; i++) {
                result.push(Document.prototype.__makeSavableCopy(doc[i], nextSchema, undefined, r));
            }
        }
        return result;
    }
                                        ...
                                        (function(key, link) {
                                            promisesLink.push(new Promise(function(resolve, reject) {
                                                r.table(self._getModel()._joins[key].link).insert(newLink, {conflict: "replace", returnChanges: true}).run().then(function(result) {
                                                    self.__proto__._links[model._joins[key].link][result.changes[0].new_val[model._joins[key].model.getTableName()+"_"+model._joins[key].rightKey]] = true;
                                                    resolve();
                                                }).error(reject);
                                            }))
                                        })(key, link);
                    ...
                    if (((key in docToSave) || (saveAll === true)) &&
                        ...
                        if (self[key] != null) {
                            ...
                            if (saveAll === true) {
                                ...
                                promises.push(self[key]._save(docToSave[model._joins[key].model.getTableName()], false, savedModel))
                            }
                        }
                    }
                }
