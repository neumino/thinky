...
function Document(model, options) {
    ...
    util.loopKeys(eventEmitter.prototype, function(emitter, key) {
        var fn = emitter[key];
        if (typeof fn === 'function') self[key] = fn.bind(self);
    });
...
Document.prototype._saveVirtual = function() {
    //TODO Copy only what we need (the virtual fields)?
    var copy = {};
    var model = this._getModel(); // instance of Model

    // We copy all the fields except the joined documents as circular references
    // could exist there.
    util.loopKeys(this, function(self, key) {
        if (model._joins[key] === undefined) {
            copy[key] = self[key];
        }
    });
...
Document.prototype._generateVirtualValues = function(doc, schema, originalDoc, virtual) {
    ...
    if (util.isPlainObject(doc) && util.isPlainObject(schema)) {
        util.loopKeys(schema, function(schema, key) {
            if (util.isPlainObject(schema[key]) && (schema[key]._type === 'virtual')) {
                ...
                else if (util.isPlainObject(virtual) && (virtual[key] !== undefined)) {
                    doc[key] = virtual[key];
                }
            }
            ...
            if ((util.isPlainObject(schema[key])) && (schema[key]._type === undefined)) {
                nextVirtual = util.isPlainObject(virtual) ? virtual[key] : undefined;
                Document.prototype._generateVirtualValues(doc[key], schema[key], originalDoc, nextVirtual)
            }
            else if ((util.isPlainObject(schema[key])) && (schema[key]._type === Object)) {
                if (util.isPlainObject(doc[key])) {
                    nextVirtual = util.isPlainObject(virtual) ? virtual[key] : undefined;
                    Document.prototype._generateVirtualValues(doc[key], schema[key].schema, originalDoc, nextVirtual)
                }
            }
            else if ((util.isPlainObject(schema[key])) && (schema[key]._type === Array)) {
                for(var i = 0; i<doc[key].length; i++) {
                    if ((util.isPlainObject(virtual)) && (Array.isArray(virtual[key]))) {
                        nextVirtual = virtual[key][i];
                    }
                    else {
                        nextVirtual = undefined;
                    }

                    if ((util.isPlainObject(schema[key].schema)) && (schema[key].schema._type === undefined)) {
                        Document.prototype._generateVirtualValues(doc[key][i], schema[key].schema, originalDoc, nextVirtual)
                    }
                    else if ((util.isPlainObject(schema[key].schema)) && (schema[key].schema._type === Object)) {
                        Document.prototype._generateVirtualValues(doc[key][i], schema[key].schema.schema, originalDoc, nextVirtual)
                    }
                }
            }
                ...
                if (Array.isArray(doc[key])) {
                    for(var i = 0; i<doc[key].length; i++) {
                        ...
                        if ((util.isPlainObject(schema[key][0])) && (schema[key][0]._type === undefined)) {
                            Document.prototype._generateVirtualValues(doc[key][i], schema[key][0], originalDoc, nextVirtual)
                        }
                        else if ((util.isPlainObject(schema[key][0])) && (schema[key][0]._type === Object)) {
                            Document.prototype._generateVirtualValues(doc[key][i], schema[key][0].schema, originalDoc, nextVirtual)
                        }
                    }
                }
            }
...
Document.prototype._generateDefault = function() {
    ...
    this.__generateDefault(this, schema, this);
    this.generateVirtualValues();
}

Document.prototype.__generateDefault = function(doc, schema, originalDoc) {
    // We need this check because the document may not be defined while the schema is
    if (util.isPlainObject(doc) && util.isPlainObject(schema)) {
        util.loopKeys(schema, function(schema, key) {
            // Generate values for undefined fields
            if (doc[key] === undefined) {
                // virtual are generated later by generateDefault
                if (util.isPlainObject(schema[key]) && (schema[key]._type !== undefined) && ((schema[key]._type !== 'virtual'))) {
                    if (typeof schema[key].default === "function") {
                        if (schema[key].default._r === undefined) {
                            doc[key] = schema[key].default.apply(originalDoc);
                        }
            ...
            // Recurse in objects and array
            if ((util.isPlainObject(schema[key])) && (schema[key]._type === undefined)) {
                Document.prototype.__generateDefault(doc[key], schema[key], originalDoc)
            }
            else if ((util.isPlainObject(schema[key])) && (schema[key]._type === Object)) {
                if (util.isPlainObject(doc[key])) {
                    Document.prototype.__generateDefault(doc[key], schema[key].schema, originalDoc)
                }
            }
            else if ((util.isPlainObject(schema[key])) && (schema[key]._type === Array)) {
                for(var i = 0; i<doc[key].length; i++) {
                    if ((util.isPlainObject(schema[key].schema)) && (schema[key].schema._type === undefined)) {
                        Document.prototype.__generateDefault(doc[key][i], schema[key].schema, originalDoc)
                    }
                    else if ((util.isPlainObject(schema[key].schema)) && (schema[key].schema._type === Object)) {
                        Document.prototype.__generateDefault(doc[key][i], schema[key].schema.schema, originalDoc)
                    }
                }
            }
            else if (Array.isArray(schema[key])) {
                if (Array.isArray(doc[key])) {
                    for(var i = 0; i<doc[key].length; i++) {
                        if ((util.isPlainObject(schema[key][0])) && (schema[key][0]._type === undefined)) {
                            Document.prototype.__generateDefault(doc[key][i], schema[key][0], originalDoc)
                        }
                        else if ((util.isPlainObject(schema[key][0])) && (schema[key][0]._type === Object)) {
                            Document.prototype.__generateDefault(doc[key][i], schema[key][0].schema, originalDoc)
                        }
                    }
                }
            }
...
Document.prototype.validateAll = function(options, modelToValidate) {
    ...
    return this._validate(options, modelToValidate, validateAll, {}, '', true)
}
...
Document.prototype._validate = function(options, modelToValidate, validateAll, validatedModel, prefix) {
    ...
    return util.hook({
        ...
        async: async,
        fn: function() {
            return self._validateHook(options, modelToValidate, validateAll, validatedModel, prefix)
        }
    })
}
Document.prototype._validateHook = function(options, modelToValidate, validateAll, validatedModel, prefix) {
    ...
    var promise = self.__validate(self, schema, prefix, docOptions, self);
...
Document.prototype.__validate = function(doc, schema, prefix, options, originalDoc) {
    ...
    if (util.isPlainObject(schema) && (schema._type !== undefined) && (util.isPlainObject(schema.options))) {
        ...
        localOptions = util.deepCopy(localOptions)
        ...
        if (schema === className) {
            ...
            if (doc === undefined) {
                if (localOptions.enforce_missing === true) {
                    schemaUtil.undefinedField(prefix);
                }
            }
            else if (doc === null) {
                if (localOptions.enforce_type === "strict") {
                    schemaUtil.strictType(prefix, typeOf);
                }
            }
                ...
                if (localOptions.enforce_type === "strict") {
                    ...
                    schemaUtil.looseType(prefix, typeOf);
                }
            }
        }
            ...
            else if (Array.isArray(schema.enum) && (util.isPlainObject(schema._enum))) {
                if (schema._enum[doc] !== true) {
    ...
    if (fieldChecked === false) { // The field is not a primitive, let's keep looking
        if ((schema === Date) || (util.isPlainObject(schema) && schema._type === Date)) {
            ...
            if (doc === undefined) {
                ...
                if (util.isPlainObject(doc) && (doc["$reql_type$"] === "TIME")) {
                    ...
                    if (date.getTime() !== date.getTime()) {
                        if (localOptions.enforce_type === "strict") {
                            schemaUtil.strictType(prefix, "date or a valid string");
                        }
                        else if (localOptions.enforce_type !== "none") {
                            schemaUtil.looseType(prefix, "date or a valid string");
                        }
                    }
                }
                    ...
                    if (localOptions.enforce_type === "strict") {
                        ...
                        schemaUtil.looseType(prefix, "date");
                    }
                }
            }
        }
        ...
        else if ((schema === Buffer) || (util.isPlainObject(schema) && schema._type === Buffer)) {
            if ((util.isPlainObject(schema)) && (typeof schema.validator === 'function')) {
                if (schema.validator(doc) === false) {
                    throw new Error("Validator for the field "+prefix+"[key] returned `false`.")
                }
            ...
            if (doc === undefined) {
                ...
                if (doc.data === undefined) {
                    schemaUtil.pseudoTypeError("binary", "data", prefix);
                }
            }
            else if ((typeof doc === 'function') && (Array.isArray(doc._query))) {
                // TOIMPROvE -- we currently just check if it's a term from the driver
                ...
                if (localOptions.enforce_type === "strict") {
                    schemaUtil.strictType(prefix, "buffer");
                }
                else if (localOptions.enforce_type !== "none") {
                    schemaUtil.looseType(prefix, "buffer");
                }
            }
        }
            ...
            if (doc === undefined) {
                ...
                if (localOptions.enforce_type === "strict") {
                    schemaUtil.strictType(prefix, "array");
                }
            }
            else if (Array.isArray(doc)) {
                ...
                if (localOptions.enforce_type === "strict") {
                    ...
                    schemaUtil.looseType(prefix, "array");
                }
            }
        }
            ...
            if (doc === undefined) {
                ...
                if (localOptions.enforce_type === "strict") {
                    schemaUtil.strictType(prefix, "object");
                }
            }
            else if (util.isPlainObject(doc)) {
                if (schema._type !== undefined) {
                    util.loopKeys(schema.schema, function(nextSchema, key) {
                        Document.prototype.__validate(doc[key], nextSchema[key], prefix+'['+key+']', localOptions, originalDoc)
                    });
                }
                else {
                    util.loopKeys(schema, function(schema, key) {
                ...
                if (localOptions.enforce_type === "strict") {
                    ...
                    schemaUtil.looseType(prefix, "object");
                }
            }
        }
    }
        ...
        util.loopKeys(doc, function(doc, key) {
            if (((doc._getModel == null) || (doc._getModel()._joins.hasOwnProperty(key) === false)) && (doc.hasOwnProperty(key)) &&
                ...
                if (localOptions.enforce_extra === 'remove') {
                    ...
                    schemaUtil.extraField(prefix, key);
                }
            }
        });
    }
...
Document.prototype.saveAll = function(docToSave, callback) {
    ...
    return self._save(docToSave, saveAll, {}, callback);
}
            ...
            var p = new Promise(function(resolve, reject) {
                            ...
                            if (self[key] != null) {
                                ...
                                (function(key) {
                                    promisesMany.push(new Promise(function(resolve, reject) {
                                        self[key]._save(docToSave[key], saveAll, savedModel).then(function() {
                    ...
                    var savedDocCb = function(result) {
                        if (result.first_error != null) {
                            ...
                            util.tryCatch(function() { // Validate the doc, replace it, and tag it as saved
                                //Document.prototype.__validate(result.changes[0].new_val, model._schema, '', model._options, self)
                        ...
                        util.tryCatch(function() {
                            ...
                            if (promise instanceof Promise) {
                                promise.then(function() {
                                    querySaveSelf.run().then(savedDocCb).error(reject)
                                }).error(reject);
                            }
                            else {
                                querySaveSelf.run().then(savedDocCb).error(reject)
                ...
                if (promises.length > 0) {
                    var cbBelongsTo = function() {
                        util.loopKeys(model._joins, function(joins, key) {
                            if (((key in docToSave) || (saveAll === true)) &&
                                (joins[key].type === 'belongsTo') && (self[key] != null)) {
