...
function Document(model, options) {
  ...
  this._options.optimisticLocking = (options.optimisticLocking != null) ? options.optimisticLocking : model.getOptions().optimisticLocking;
...
Document.prototype._saveSelf = function(
  ...
  var buildQuery = function () {
    if (self.__proto__._saved === false) {
      var additionalInformationForCopy = {};
      if (model._options.optimisticLocking) {
        additionalInformationForCopy[model._version] = 0;
      }
      return querySaveSelf = r.table(constructor.getTableName())
          .insert(Object.assign(copy, additionalInformationForCopy), {returnChanges: 'always'})
    }
    else {
      ...
      if (model._options.optimisticLocking) {
        return querySaveSelf = r.table(constructor.getTableName())
            .get(copy[model._pk]).replace(function(row) {
              const newVersion = {};
              newVersion[model._version] = self[model._version]+1;
              return r.branch(row(model._version).eq(self[model._version]), Object.assign(copy, newVersion), r.error("Optimistic Locking failed"));
