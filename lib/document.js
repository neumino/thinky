...
Document.prototype._makeSavableCopy = function() {
    ...
    return this.__makeSavableCopy(this, schema, model)
}
// __makeSavableCopy
Document.prototype.__makeSavableCopy = function(doc, schema, model) {
    //TODO Make sure that schema.foo won't throw

    // model is an instance of a Model, or undefined
    ...
    if (util.isPlainObject(doc) && (doc instanceof Buffer === false)) {
        ...
        for(var i=0; i<keys.length; i++) {
            ...
            if ((util.isPlainObject(model) === false) || (model._joins[key] === undefined)) { // We do not copy joined documents
                ...
                if (copyFlag === true) {
                    result[key] = Document.prototype.__makeSavableCopy(doc[key], nextSchema);
                }
            }
        }
        return result;
        ...
        if (copyFlag === true) {
            for(var i=0; i<doc.length; i++) {
                result.push(Document.prototype.__makeSavableCopy(doc[i], nextSchema));
            }
        }
        return result;
    }
            ...
            var p = new Promise(function(resolve, reject) {
                ...
                var saveSelf = function() {
                    ...
                    if (self.__proto__._saved === false) {
                            ...
                            util.tryCatch(function() {
                                ...
                                if (promise instanceof Promise) {
                                    promise.then(function() {
                                        r.table(constructor.getTableName()).insert(copy, {returnChanges: true}).run().then(savedDocCb).error(reject)
                                    }).error(reject);
                                }
                                else {
                                    r.table(constructor.getTableName()).insert(copy, {returnChanges: true}).run().then(savedDocCb).error(reject)
                        ...
                        r.table(constructor.getTableName()).get(copy[model._pk]).replace(copy, {returnChanges: true}).run().then(function(result) {
                            if (result.first_error != null) {
                                reject(new Error(result.first_error));
                            }
                            else {
