        ...
        for(var key in this._getModel()._joins) {
            if (this._getModel()._joins[key].type === 'hasOne') {
                ...
                if (this[key] instanceof Document) {
                    ...
                    this[key].__proto__._belongTo[this._getModel()._name] = {
                        doc: this,
                ...
                for(var i=0; i<this[key].length; i++) {
                    if (this[key][i] instanceof Document) {
                        ...
                        this[key][i].__proto__._belongLinks[this._getModel()._name] = {
                            doc: this,
                            foreignKey: this._getModel()._joins[key].leftKey,
                            key: key
                        };
                ...
                Promise.all(promises).then(function() {
                    for(var key in model._joins) {
                        if (((model._joins[key].model.getName() in modelToSave) || (saveAll === true)) &&
                            ...
                            // Save the document that belongs to self[key]
                            self[key].__proto__._belongTo[model.getName()] = {
                                doc: self,
...
Document.prototype.delete = function(purge) {
    ...
    // doc.otherDoc.delete()
    for(var key in self.__proto__._belongTo) {
        delete self.__proto__._belongTo[key].doc[self.__proto__._belongTo[key].key];
        delete self.__proto__._belongTo[key].doc[self.__proto__._belongTo[key].foreignKey];
        
        if (purge === false) {
            var arg = {};
            arg[key] = true;
            promises.push(self.__proto__._belongTo[key].doc.save(arg));
        }
    ...
    //
    for(var key in self.__proto__._belongLinks) {
        var parentDoc = self.__proto__._belongLinks[key].doc;
        var field = [self.__proto__._belongLinks[key].key];
    ...
    if (purge !== false) {
        ...
        for(var modelName in self._getModel()._reverseJoins) {
            if (self._getModel()._reverseJoins[modelName].type === 'belongsTo') {
                var reverseJoin = self._getModel()._reverseJoins[modelName];
                promises.push(
