...
Document.prototype._generateDefault = function() {
    ...
    var options = self.getModel().getOptions(); // TODO Check
...
Document.prototype.validate = function(options) {
    options = options || {};
    var modelOptions = this.getModel()._options;
    var schema = this.getModel()._schema;
...
Document.prototype.saveAll = function() {
    return this._save({hasOne: true, hasMany: true, belongsTo: true, manyToMany: true});
}
Document.prototype._save = function() {
    var self = this;

    var model = self.getModel(); // instance of Model
    ...
    if (self.__proto__._saved === false) {
        var p = new Promise(function(resolve, reject) {
            if (self.__proto__._model._tableReady === true) {
                r.table(model._name).insert(self, {returnVals: true}).run().then(function(result) {
                    try{
                        Document.prototype._validate(result, model._schema, '')
                ...
                model._onTableReady.push({
                    query: r.table(model._name).insert(self, {returnVals: true}),
                    resolve: resolve,
                    reject: reject
                })
            }
        ...
        return r.table(model._name).get().update(this, {returnVals: true}).run()
    }
}
