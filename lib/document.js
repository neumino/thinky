...
var schemaUtil = require(__dirname + '/schema.js'),
    type = require(__dirname + '/type/index.js'),
...
function Document(model, options) {
  ...
  if (util.isPlainObject(options)) {
    ...
    this._schemaOptions.enforce_missing =
        util.exists(options.enforce_missing) ? options.enforce_missing : model.getOptions().enforce_missing;
    this._schemaOptions.enforce_extra =
        util.exists(options.enforce_extra) ? options.enforce_extra : model.getOptions().enforce_extra;
    this._schemaOptions.enforce_type =
        util.exists(options.enforce_type) ? options.enforce_type : model.getOptions().enforce_type;
  }
  ...
  this._options.timeFormat =
    util.exists(options.timeFormat) ? options.timeFormat : model.getOptions().timeFormat;
  this._options.validate =
    util.exists(options.validate) ? options.validate : model.getOptions().validate;
    ...
    if (util.recurse(key, joins, modelToValidate, validateAll, validatedModel)) {
      if (((joins[key].type === 'hasOne') || (joins[key].type === 'belongsTo'))) {
        if (util.isPlainObject(self[key])) {
          ...
          promise = self[key].validate(options, modelToValidate[key], validateAll, validatedModel, prefix + '[' + key + ']');
          if (promise instanceof Promise) {
            promises.push(promise);
            promise = null;
          }
        }
        else if (util.exists(self[key])) {
...
Document.prototype._onSavedBelongsTo = function(
  ...
  util.loopKeys(belongsToKeysSaved, function(unusedJoins, key) {
    var joins = model._joins;
    if (util.exists(self[key])) {
      self.__proto__._belongsTo[key] = true;

      // Copy foreign key
      if (!util.exists(self[key][joins[key].rightKey])) {
      ...
      if (!util.exists(self[key].__proto__._parents._belongsTo[constructor.getTableName()])) {
...
Document.prototype._onSaved = function(result, docToSave, saveAll, savedModel, resolve, reject) {
  ...
  if (util.exists(result.first_error)) {
...
Document.prototype._saveMany = function(docToSave, saveAll, savedModel, resolve, reject) {
  ...
  util.loopKeys(model._joins, function(joins, key) {
    if (((key in docToSave) || (saveAll === true)) &&
        ...
        (function(_key) {
          promisesMany.push(new Promise(function(resolve, reject) {
            self[_key]._save(docToSave[_key], saveAll, savedModel).then(function() {
              ...
              if (!util.exists(self[_key].__proto__._parents._hasOne[self._getModel()._name])) {
      ...
      else if (!util.exists(self[key]) && util.exists(self.__proto__._hasOne[key])) {
  ...
  util.loopKeys(model._joins, function(joins, key) {
    if (((key in docToSave) || (saveAll === true)) &&
      ...
      if (Array.isArray(self[key])) {
        for (var i = 0; i < self[key].length; i++) {
          if (util.exists(self[key][i][joins[key].model._pk])) {
            pkMap[self[key][i][joins[key].model._pk]] = true;
          }
        }
      }

      if (util.exists(self.__proto__._hasMany[key])) {
        for (var j = 0; j < self.__proto__._hasMany[key].length; j++) {
          if (!util.exists(pkMap[self.__proto__._hasMany[key][j].doc[[joins[key].model._pk]]])) {
            delete self.__proto__._hasMany[key][j].doc[self.__proto__._hasMany[key][j].foreignKey];
            promisesMany.push(self.__proto__._hasMany[key][j].doc._save(docToSave[key], saveAll, savedModel));
          }
        }
      }
      self.__proto__._hasMany[key] = [];

      self[key].forEach(function(doc) {
        doc[joins[key].rightKey] = self[joins[key].leftKey];

        promisesMany.push(new Promise(function(resolve, reject) {
          doc._save(docToSave[key], saveAll, savedModel).then(function(doc) {
            ...
            if (!util.exists(doc.__proto__._parents._hasMany[self._getModel()._name]))
              doc.__proto__._parents._hasMany[self._getModel()._name] = [];

            doc.__proto__._parents._hasMany[self._getModel()._name].push({
              doc: self,
              key: key
            });
  ...
  util.loopKeys(model._joins, function(joins, key) {
    ...
    if (((key in docToSave) || (saveAll === true)) &&
      ...
      self[key].forEach(function(doc) {
        ...
        promisesMany.push(new Promise(function(resolve, reject) {
          doc._save(docToSave[key], saveAll, savedModel).then(function() {
            // self.__proto__._links will be saved in saveLinks
            if (!util.exists(doc.__proto__._parents._belongsLinks[self._getModel()._name]))
              doc.__proto__._parents._belongsLinks[self._getModel()._name] = [];

            doc.__proto__._parents._belongsLinks[self._getModel()._name].push({
              doc: self,
              key: key
            });
            resolve();
  ...
  if (all === true) {
    util.loopKeys(self._getModel()._joins, function(joins, key) {
      if (joins[key].type === 'hasOne') {
        ...
        if (Array.isArray(self[key])) {
          for (var j = 0; j < self[key].length; j++) {
            if (self[key][j] instanceof Document) {
              self[key][j].setSaved(true);
            }
          }
        }
      }
    ...
    util.loopKeys(self._getModel()._joins, function(joins, key) {
      if (util.exists(self[key])) {
        if (joins[key].type === 'hasOne') {
          ...
          if (!util.exists(self[key].__proto__._parents._hasOne[self._getModel()._name])) {
          ...
          if (!util.exists(self[key].__proto__._parents._belongsTo[self._getModel()._name])) {
          ...
          for (var i = 0; i < self[key].length; i++) {
            ...
            if (!util.exists(self[key][i].__proto__._parents._hasMany[self._getModel()._name])) {
          ...
          for (var j = 0; j < self[key].length; j++) {
            if (self[key][j].isSaved()) {
              self.__proto__._links[self._getModel()._joins[key].link][self[key][j][self._getModel()._joins[key].rightKey]] = true;
            }

            if (!util.exists(self[key][j].__proto__._parents._belongsLinks[self._getModel()._name])) {
              self[key][j].__proto__._parents._belongsLinks[self._getModel()._name] = [];
            }
            self[key][j].__proto__._parents._belongsLinks[self._getModel()._name].push({
              doc: self,
              key: key
            });
...
Document.prototype._deleteHook = function(docToDelete, deleteAll, deletedDocs, deleteSelf, updateParents, callback) {
  ...
  util.loopKeys(self._getModel()._joins, function(joins, key) {
    ...
    if ((joins[key].type === 'hasAndBelongsToMany') && (Array.isArray(self[key]))) {
      ...
      for (var j = 0; j < self[key].length; j++) {
        if (((self[key][j] instanceof Document) && (self[key][j].isSaved() === true))
          && ((key in docToDelete) || ((deleteAll === true) && (deletedDocs.indexOf(self[key][j]) === -1)))) {

          //pks.push(self[key][i][joins[key].model._getModel()._pk]);
          docsToDelete.push(self[key][j]);
          // We are going to do a range delete, but we still have to recurse
          promises.push(self[key][j]._delete(docToDelete[key], deleteAll, deletedDocs, true, false));
          ...
          if (self.getModel()._getModel()._pk === joins[key].leftKey) {
            ...
            if (self._getModel()._name < joins[key].model._getModel()._name) {
              linksPks.push(self[joins[key].leftKey] + '_'+ self[key][j][joins[key].rightKey]);
            }
            else {
              linksPks.push(self[key][j][joins[key].rightKey] + '_'+ self[joins[key].leftKey]);
            }
          }
        }
        else if ((self[key][j] instanceof Document) && (deletedDocs.indexOf(self[key][j]) === -1)) {
...
Document.prototype._emitRetrieve = function() {
  ...
  util.loopKeys(self._getModel()._joins, function(joins, key) {
    if ((joins[key].type === 'hasOne') || (joins[key].type === 'belongsTo')) {
      if (util.exists(self[key]) && (typeof self[key]._emitRetrieve === 'function')) {
