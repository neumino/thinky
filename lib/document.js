...
Document.prototype._save = function(options, modelToSave, savedModel) {
    ...
    var p = new Promise(function(resolve, reject) {
        // Save belongsTo
        // Save this
        // Save hasOne, hasMany and manyToMany docs
        // Save link


        var saveLinks = function() {
            // All the manyTomany were saved
            resolve(self);
            ...
            if (options.manyToMany === true) {
                for(var key in model._joins) {
                    if ((model._joins[key].type === 'manyToMany') && (Array.isArray(self[key]))) {
                        ...
                        if (self.__proto__.links[model._joins[key].link] === undefined) {
                            self.__proto__.links[model._joins[key].link] = {}
                        }
                        var oldKeys = self.__proto__.links[model._joins[key].link];
                        ...
                        for(link in newKeys) {
                            if (oldKeys[link] !== true) {
                                ...
                                newLink[constructor.getName()] = self[model._joins[key].leftKey]
                                newLink[model._joins[key].model.getName()] = link
                                promisesLink.push(
                                    r.table(self.__proto__.links[model._joins[key].link]).insert(newLink, {upsert: true}).run()
                                )
                            }
                        }
                        ...
                        if (keysToDelete.length > 0) {
                            var table = r.table(self.__proto__.links[model._joins[key].link])
                            promisesLink.push(
                        ...
                        if (promisesLinks.length > 0) {
                            Promise.all(promisesLinks).then(function() {
                                resovle(self)
                            }).error(reject);
                        }
                        else {
            ...
            if (options.hasOne === true) {
                for(var key in model._joins) {
                    if ((model._joins[key].type === 'hasOne') && (self[key] != null)) {
                        ...
                        promisesMany.push(self[key].saveAll(options))
                    }
                }
            }
                ...
                for(var key in model._joins) {
                    if ((model._joins[key].type === 'hasMany') && (Array.isArray(self[key]))) {
                        for(var i=0; i<self[key].length; i++) {
                            ...
                            promisesMany.push(self[key][i].saveAll(options))
                        }
                    }
                }
            }
        ...
        var saveSelf = function() {
            if (self.__proto__._saved === false) {
                ...
                if (self.__proto__._model._tableReady === true) {
                    ...
                    model._getModel()._onTableReady.push({
        ...
        var promises = []
        if (options.belongsTo === true) {
            for(var key in model._joins) {
                if ((model._joins[key].type === 'belongsTo') && (self[key] != null)) {
        ...
        // Save the manyToMany joined documents
