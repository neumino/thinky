...
Query.prototype.getJoin = function(modelToGet, getAll, gotModel) {
    ...
    gotModel = gotModel || {};
    gotModel[model.getName()] = true;


    for(var key in joins) {
        if (((getAll === true) || (key in modelToGet)) && ((getAll === false) || (gotModel[joins[key].model.getName()] !== true))) {
            if (joins[key].type === 'hasOne') {
                ...
                self._query = self._query.merge(function(doc) {
                    if ((modelToGet[key] != null) && (modelToGet[key]._order != null)) {
                        //return r.object(key, r.table(joins[key].model.getName()).getAll(doc(joins[key].leftKey), {index: joins[key].rightKey}).coerceTo("ARRAY").orderBy(options[key].order))
                        return r.object(key,
                            new Query(joins[key].model,
                                r.table(joins[key].model.getName())
                ...
                self._query = self._query.merge(function(doc) {
                    if ((modelToGet[key] != null) && (modelToGet[key]._order != null)) {
                        ...
                        */
                        return r.object(key,
                            new Query(joins[key].model,
                                r.table(joins[key].link).getAll(doc(joins[key].leftKey), {index: model.getName()+"_"+joins[key].leftKey}).concatMap(function(link) {
                                return r.table(joins[key].model.getName()).getAll(link(joins[key].model.getName()+"_"+joins[key].rightKey), {index: joins[key].rightKey})
