        ...
        if (((getAll === true) || (key in modelToGet)) && ((getAll === false) || (gotModel[joins[key].model.getTableName()] !== true))) {
            if (joins[key].type === 'hasOne') {
                self._query = self._query.merge(function(doc) {
                    return r.table(joins[key].model.getTableName()).getAll(doc(joins[key].leftKey), {index: joins[key].rightKey}).coerceTo("ARRAY").do(function(result) {
                        return r.branch(
                            result.count().eq(1),
                ...
                self._query = self._query.merge(function(doc) {
                    if ((modelToGet[key] != null) && (modelToGet[key]._order != null)) {
                        //return r.object(key, r.table(joins[key].model.getTableName()).getAll(doc(joins[key].leftKey), {index: joins[key].rightKey}).coerceTo("ARRAY").orderBy(options[key].order))
                        return r.object(key,
                        ...
                        //return r.object(key, r.table(joins[key].model.getTableName()).getAll(doc(joins[key].leftKey), {index: joins[key].rightKey}).coerceTo("ARRAY"))
                        return r.object(key,
                            new Query(joins[key].model,
                                r.table(joins[key].model.getTableName())
                ...
                self._query = self._query.merge(function(doc) {
                    if ((modelToGet[key] != null) && (modelToGet[key]._order != null)) {
                        /*
                        return r.object(key, r.table(joins[key].link).getAll(doc(joins[key].leftKey), {index: model.getTableName()+"_"+joins[key].leftKey}).concatMap(function(link) {
                        ...
                        }).coerceTo("ARRAY").orderBy(options[key].order))
                        */
                        return r.object(key,
                            new Query(joins[key].model,
