            ...
            if (joins[key].type === 'hasOne') {
                ...
                self._query = self._query.merge(function(doc) {
                    if ((model.getTableName() === joins[key].model.getTableName()) && (joins[key].leftKey === joins[key].rightKey)) {
                        ...
                        var mergeLink = modelToGet[key] && modelToGet[key]._merge || false;
                        innerQuery = r.table(joins[key].link).getAll(doc(joins[key].leftKey), {index: joins[key].leftKey+"_"+joins[key].leftKey}).concatMap(function(link) {
                            var docs = r.table(joins[key].model.getTableName()).getAll(
                                r.branch(
                            ...
                            return r.branch(mergeLink,
                                docs.map(function(doc) {
                                    return doc.merge(link.without('id', joins[key].leftKey+"_"+joins[key].leftKey));
                                }),
                                docs);
                        });
                        ...
                        innerQuery = r.table(joins[key].link).getAll(doc(joins[key].leftKey), {index: model.getTableName()+"_"+joins[key].leftKey}).concatMap(function(link) {
                            var docs = r.table(joins[key].model.getTableName()).getAll(link(joins[key].model.getTableName()+"_"+joins[key].rightKey), {index: joins[key].rightKey});
                            return r.branch(mergeLink,
                                docs.map(function(doc) {
                                    return doc.merge(link.without('id', model.getTableName()+"_"+joins[key].leftKey, joins[key].model.getTableName()+"_"+joins[key].rightKey));
                                }),
                                docs);
                        });
