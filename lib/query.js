...
Query.prototype.execute = function() {
    ...
    if (self._model._error !== null) {
        ...
        p = new Promise(function(resolve, reject) {
            self._model.on('ready', function() {
                self._query.execute().then(function(result) {
...
Query.prototype.getJoin = function(modelToGet) {
    ...
    for(var key in joins) {
        ...
        if (((joins[key].model.getName() in modelToGet) || (getAll === true)) && (joins[key].type === 'manyToMany')) {
            self._query = self._query.merge(function(doc) {
                return r.object(key, r.table(joins[key].link).getAll(doc(joins[key].leftKey), {index: model.getName()+"_"+joins[key].leftKey}).coerceTo("ARRAY"))
                    /*.concatMap(function(link) {
                    //TODO Check the length of the array?
                    return r.table(joins[key].model.getName()).getAll(link(joins[key].rightKey), {index: joins[key].rightKey}).coerceTo("ARRAY")
                }).coerceTo("ARRAY"))
                */
