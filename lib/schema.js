...
function generateVirtual(doc, defaultField, originalDoc, virtual) {
    ...
    if (path[j] === arrayPrefix) {
      if (!Array.isArray(field)) {
        ...
        for(var k=0; k<field.length; k++) {
          ...
          generateVirtual(field[k], {path: defaultField.path.slice(j+1), value: defaultField.value}, originalDoc, virtualValue);
        }
      }
      keepGoing = false;
    }
  ...
  if (keepGoing) {
    if (value === undefined) {
      ...
      field[path[path.length-1]] = value.call(doc, originalDoc);
    }
    else {
