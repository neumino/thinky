...
function Thinky(config) {
  ...
  self._options.enforce_missing =
      (util.exists(config.enforce_missing)) ? config.enforce_missing : false;
  self._options.enforce_extra =
      (util.exists(config.enforce_extra)) ? config.enforce_extra : 'none';
  self._options.enforce_type =
      (util.exists(config.enforce_type)) ? config.enforce_type : 'loose';

  // Format of time objects returned by the database, by default we convert
  // them to JavaScript Dates.
  self._options.timeFormat =
      (util.exists(config.timeFormat)) ? config.timeFormat : 'native';
  // Option passed to each model we are going to create.
  self._options.validate =
      (util.exists(config.validate)) ? config.validate : 'onsave';
