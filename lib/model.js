...
Model.prototype.hasMany = function(joinedModel, fieldDoc, leftKey, rightKey, options) {
    ...
    this._getModel()._joins[joinedModel._getName()] = {
        model: joinedModel,
        fieldDoc: fieldDoc,
        leftKey: leftKey,
        rightKey: rightKey,
 ...
 *
 * TODO - Do we want that -- A hasAndBelongsToMany is symmetric and the symmetrical relation is automatically created
 *
...
Model.prototype.hasAndBelongsToMany = function(joinedModel, fieldDoc, leftKey, rightKey, options) {
    ...
    r.branch(
        ...
        r.table(link).indexList().contains(self.getName()+'_'+leftKey).do(function() {
            return r.table(link).indexCreate(self.getName()+'_'+leftKey)
        }),
        r.tableCreate(link).do(function(result) {
        return r.expr([
            r.table(link).indexCreate(self.getName()+'_'+leftKey)
            r.table(link).indexCreate(joinedModel.getName()+'_'+rightKey)
        ]).do(function() {
            return r.table(link).indexWait(self.getName()+'_'+leftKey, joinedModel.getName()+'_'+rightKey)
        })
    })
    //TODO create table with indexes
