...
function Model(name, schema, options, thinky) {
    ...
    this._pk = (typeof options.pk != null) ? options.pk : 'id';
    ...
    this._onTableReady = []; // {query, resolve, reject, execute}
...
Model.prototype._setReady = function() {
    ...
    for(var i = 0; i<this._onTableReady.length; i++) {
        if (this._onTableReady[i].execute === true) {
            this._onTableReady[i].query.execute().then(this._onTableReady[i].resolve).error(this._onTableReady[i].reject);
        }
        else {
...
/*
// List of keys, we probably will have to track sequence/object/primitive
dbCreate dbDrop dbList tableCreate tableDrop tableList indexList indexCreate indexDrop indexStatus indexWait insert update replace delete sync db table
get getAll between filter innerJoin outerJoin eqJoin zip map withFields concatMap orderBy desc asc skip limit slice
nth indexesOf isEmpty union sample reduce count distinct group ungroup contains sum avg min max row pluck without
merge literal append prepend difference setInsert setUnion setIntersection setDifference getField hasFields insertAt spliceAt deleteAt changeAt keys object match upcase downcase
add sub mul div mod and or eq ne gt ge lt le not now time epochTime ISO8601 inTimezone timezone during date timeOfDay year month day dayOfYear dayOfWeek hours minutes seconds toISO8601 toEpochTime monday tuesday wednesday
thursday friday saturday sunday january february march april may june july august september october november december
do branch forEach error default expr js coerceTo typeOf info json exprJSON toString
*/
