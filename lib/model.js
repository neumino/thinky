    ...
    if (options.init !== false) {
        ...
        var queryIndex2 = r.branch(
            ...
            r.table(joinedModel.getName()).indexCreate(rightKey).do(function() {
                return r.table(joinedModel.getName()).indexWait(leftKey)
            })
        )
        ...
        else if (joinedModel._getModel()._tableCreated === true) {
            joinedModel._getModel()._onTableCreated.push({
                query: query,
                resolve: successCb,
                reject: errorCb
