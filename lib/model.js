...
function Model(name, schema, options, thinky) {
  ...
  this._options.version = (options.version != null) ? options.version : thinky._options.version;
  ...
  this._version = this._options.version ? this._options.version : undefined;
...
Model.prototype.validateAll = function(docs, options, modelToValidate) {
  var validateAll = modelToValidate === undefined;
  modelToValidate = modelToValidate || {};

  return this.validate(docs, options, modelToValidate, validateAll, {}, '', true);
}

Model.prototype.validate = function(docs, options, modelToValidate, validateAll, validatedModel, prefix) {
  var self = this;
  var r = self._getModel()._thinky.r;
  ...
  var p = new Promise(function(mainResolve, mainReject) {
    ...
    for(var i = 0; i < docs.length; i++) {
      ...
      util.tryCatch(function() {
        promise = docs[i].validate(options, modelToValidate, validateAll, validatedModel, prefix);
...
Model.prototype.saveMultiple = function(docs) {
  return this._saveMultiple(docs, {}, false, {});
}

Model.prototype.saveAllMultiple = function(docs, docToSave) {
  var saveAll = docToSave === undefined;
  docToSave = docToSave || {};

  return this._saveMultiple(docs, docToSave, saveAll, {});
}

Model.prototype._saveMultiple = function(docs, docToSave, saveAll, savedModel) {
  var self = this;
  var r = self._getModel()._thinky.r;
  var isArray = Array.isArray(docs);
  ...
  var p = new Promise(function(mainResolve, mainReject) {
    ...
    if (foundNonValidDoc === false) {
      Promise.all(promises).then(function() {
        var promises = [];
        for(var i = 0; i < docs.length; i++) {
          promises.push(docs[i]._save(docToSave, saveAll, savedModel));
        }
        Promise.all(promises).then(function() {
          mainResolve(docs);
        }).error(function(error) {
  ...
  var p = new Promise(function(mainResolve, mainReject) {
    ...
    var executeInsert = function(resolve, reject) {
      ...
      if (toSave === 0) {
        ...
        // If versioning is enabled, we need to calculate the new versions
        if (self._getModel()._version) {
          for(var i = 0; i < copies.length; i++) {
            copies[i][self._getModel()._version] = docs[i]._calculateVersion();
          }
        }
        var _options;
      ...
      util.tryCatch(function() {
        for(var i = 0; i < data.length; i++) {
          ...
          (function(i) {
            ...
            if (!data[i]._getModel().getOptions().validate === 'onlyOnSave') {
              if (promise instanceof Promise) {
              ...
              if (Array.isArray(data.data[i][1])) {
                for(var j = 0; j < data.data[i][1].length; j++) {
                  (function(j) {
                    ...
                    if (!newDoc._getModel().getOptions().validate === 'onlyOnSave') {
                      if (promise instanceof Promise) {
                        promise.then(function() {
