...
function Model(name, schema, options, thinky) {
  ...
  options = options || {};
  this._options = {};
  this._options.enforce_missing = util.exists(options.enforce_missing) ? options.enforce_missing : thinky._options.enforce_missing;
  this._options.enforce_extra = util.exists(options.enforce_extra) ? options.enforce_extra : thinky._options.enforce_extra;
  this._options.enforce_type = util.exists(options.enforce_type) ? options.enforce_type : thinky._options.enforce_type;
  this._options.timeFormat = util.exists(options.timeFormat) ? options.timeFormat : thinky._options.timeFormat;
  this._options.validate = util.exists(options.validate) ? options.validate : thinky._options.validate;
  ...
  this._pk = (util.exists(options.pk)) ? options.pk : 'id';
      ...
      if (util.exists(doc[key])) {
        if ((joins[key].type === 'hasOne') && (doc[key] instanceof Document === false)) {
          ...
          for (var j = 0; j < doc[key].length; j++) {
            if (doc[key][j] instanceof Document === false) {
              doc[key][j] = new joins[key].model(doc[key][j], options);
            }
          }
        }
      }
        ...
        r.table(self.getTableName()).insert(copies, _options).run().then(function(results) {
          if (results.errors === 0) {
            ...
            rejects.forEach(function(reject) {
              reject(new Error('An error occurred during the batch insert. Original results:\n'+ JSON.stringify(results, null, 2)));
            });
          }
        }).error(reject);
      }
...
Model.prototype.docAddListener = function(eventKey, listener) {
  ...
  if (!util.exists(listeners[eventKey])) {
    listeners[eventKey] = [];
  }
